...
func (jam *JWTAuthMiddleware) Authenticate() fiber.Handler {
	...
	return func(c *fiber.Ctx) error {
		...
		if authHeader == "" {
			jam.log.Debug().Msg("Authorization header missing")
		...
		if len(parts) != 2 || strings.ToLower(parts[0]) != "bearer" {
			jam.log.Debug().Str("header", authHeader).Msg("Authorization header format must be Bearer {token}")
		...
		if err != nil {
			...
			jam.log.Info().Err(err).Msg("Invalid or expired access token")
		...
		if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
			...
			if !ok || userIDStr == "" {
				jam.log.Debug().Interface("claims", claims).Msg("Invalid token: 'sub' claim is missing or not a string")
				jam.log.Error().Err(err).Msg("Invalid access token claims")
			...
			jam.log.Warn().Interface("Authenticate", "JWTAuthMiddleware").Bool("tokenValid", token.Valid).Msg("Token claims invalid or token is not valid")
