...
func (acm *AuthContextMiddleware) AddUserContext() fiber.Handler {
	...
	return func(c *fiber.Ctx) error {
		...
		if !ok || userIDStr == "" {
			...
			acm.logger.Error().Msg("Invalid user id from context")
		...
		if err != nil {
			if errors.Is(err, err) { // Use specific errors
				...
				acm.logger.Error().Err(err).Msg("Unauthorized: User not found")
				return c.Status(fiber.StatusUnauthorized).JSON(utils.ErrorResponse("User not found"))
			} else {
				// Log the actual error
				//http.Error(w, "Internal Server Error", http.StatusInternalServerError)
				acm.logger.Error().Err(err).Msg("Error while getting user")
